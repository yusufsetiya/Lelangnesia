...
class M_penawaran extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT * FROM tb_lelang JOIN tb_barang ON tb_lelang.id_barang=tb_barang.id_barang WHERE tb_barang.status_barang='1'")->result();
        return $query;
    }

    public function get_selesai()
    {
        $query = $this->db->query("SELECT * FROM tb_lelang JOIN tb_barang ON tb_lelang.id_barang=tb_barang.id_barang WHERE tb_barang.status_barang='2' OR tb_barang.status_barang='3'")->result();
        return $query;
    }
    ...
    public function get_tawaran($where)
    {
        ...
        // return $query;
        $sql = "SELECT * FROM history_lelang JOIN tb_lelang ON history_lelang.id_lelang=tb_lelang.id_lelang JOIN tb_barang ON history_lelang.id_barang=tb_barang.id_barang JOIN tb_masyarakat ON history_lelang.id_user=tb_masyarakat.id_user WHERE history_lelang.id_lelang =$where ORDER BY convert(history_lelang.penawaran_harga,decimal) DESC";
        return $this->db->query($sql)->result();
    ...
    public function get_tertinggi($where)
    {
        // $query = $this->db->query("SELECT * FROM history_lelang JOIN tb_lelang ON history_lelang.id_lelang=tb_lelang.id_lelang JOIN tb_barang tb_lelang.id_barang=tb_barang.id_barang WHERE id_lelang = $where")->result();
        // return $query;
        $sql = "SELECT MAX(convert(penawaran_harga,decimal)) AS tertinggi FROM history_lelang WHERE id_lelang =$where";
    ...
    {
        $sql = "SELECT email AS eml FROM tb_masyarakat WHERE id_user=$where";
        return $this->db->query($sql)->row_array();
        // return $this->db->get_where($lelang, $where);
        // return $this->db->get('tb_petugas')->result();
    }
    public function get_nmBrg($id_barang)
    {
        $sql = "SELECT nama_barang AS brg FROM tb_barang WHERE id_barang=$id_barang";
        return $this->db->query($sql)->row_array();
        // return $this->db->get_where($lelang, $where);
        // return $this->db->get('tb_petugas')->result();
    }
